...
if ( ! function_exists( 'dessertstorm_posted_on' ) ) :
/**
...
function dessertstorm_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'dessertstorm' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'dessertstorm' ),
...
if ( ! function_exists( 'dessertstorm_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function dessertstorm_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'dessertstorm' ) ); // WPCS: XSS OK.
		if ( $categories_list && dessertstorm_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'dessertstorm' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'dessertstorm' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'dessertstorm' ) . '</span>', $tags_list );
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'dessertstorm' ), esc_html__( '1 Comment', 'dessertstorm' ), esc_html__( '% Comments', 'dessertstorm' ) );
		echo '</span>';
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'dessertstorm' ),
